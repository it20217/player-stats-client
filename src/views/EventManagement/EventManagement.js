...
function EventManagement() {
  ...
  async function submitNewAssignment() {
    const response = await fetch(`${REACT_APP_BASE_URL}/POST/assignment`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: {
        Accept: "application/json",
  ...
  /** Code block begins. Delete player assignment */
  async function removeAssignment(assignmentId) {
    const response = await fetch(`${REACT_APP_BASE_URL}/DELETE/assignments/${assignmentId}`, {
      method: 'DELETE',
      credentials: 'same-origin',
      headers: {
        Accept: "application/json",
  ...
  useEffect(()=> {
    const getEvents = async()=> {
      try {
        const response = await fetch(`${REACT_APP_BASE_URL}/events`);
    ...
    getEvents();
  }, [reload, REACT_APP_BASE_URL])

  useEffect(()=> {
    const getDataset = ()=> {
      try {
        fetch(`${REACT_APP_BASE_URL}/events/dataset`)
        .then(res => res.json())
        .then(data =>       
          setDataset({
      ...
      <div className="py-12 min-h-screen">
          ...
          <div className="flex flex-col lg:flex-row p-8 justify-between items-start lg:items-stretch w-full">
            ...
            <div className="w-full lg:w-3/4 xl:w-2/3 flex flex-col lg:flex-row items-start lg:items-center justify-between">
              <div className="relative w-full lg:w-1/4 my-2 lg:my-0 lg:mx-2 xl:mx-4 z-10">
                ...
                <select disabled aria-label="Selected tab" className="relative z-10 cursor-pointer focus:outline-none focus:border-gray-800 focus:shadow-outline-gray text-sm form-select block w-full py-2 px-2 xl:px-3 border border-gray-300 dark:border-gray-200 rounded text-gray-600 dark:text-gray-400 appearance-none bg-transparent">
                  <option key={"r-1"} className="text-sm text-gray-600 dark:text-gray-400">Representatives</option>
                  <option key={"r-2"} className="text-sm text-gray-600 dark:text-gray-400">Representatives</option>
                  <option key={"r-3"} className="text-sm text-gray-600 dark:text-gray-400">
                    Representatives
                  </option>
                  <option key={"r-4"} className="text-sm text-gray-600 dark:text-gray-400">Representatives</option>
                  <option key={"r-5"} className="text-sm text-gray-600 dark:text-gray-400">Representatives</option>
                </select>
              </div>
              <div className="relative w-full lg:w-1/4 my-2 lg:my-0 lg:mx-2 xl:mx-4 z-10">
                ...
                <select disabled aria-label="Selected tab" className="relative z-10 cursor-pointer focus:outline-none focus:border-gray-800 focus:shadow-outline-gray text-sm form-select block w-full py-2 px-2 xl:px-3 border border-gray-300 dark:border-gray-200 rounded text-gray-600  dark:text-gray-400  appearance-none bg-transparent">
                  <option key={"s-1"} className="text-sm text-gray-600 dark:text-gray-400">Status Reports</option>
                  <option key={"s-2"} className="text-sm text-gray-600 dark:text-gray-400">Status Reports</option>
                  <option key={"s-3"} className="text-sm text-gray-600 dark:text-gray-400">
                    Status Reports
                  </option>
                  <option key={"s-4"} className="text-sm text-gray-600 dark:text-gray-400">Status Reports</option>
                  <option key={"s-5"} className="text-sm text-gray-600 dark:text-gray-400">Status Reports</option>
                </select>
              </div>
              ...
              <table className="w-full bg-white dark:bg-gray-800 rounded">
                <thead>
                  <tr className="w-full h-16 border-gray-300 dark:border-gray-200 border-b py-8 bg-slate-100">
                    ...
                    <th className="whitespace-nowrap text-gray-600 font-normal pr-6 text-left text-sm tracking-normal leading-4 text-center">Action</th>
                    <td className="pr-8">
                    ...
                    <tr className="w-full h-24 border-gray-300 border-b border-t hover:border-indigo-300 hover:shadow-md cursor-pointer transition duration-150 ease-in-out">
                      ...
                      <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4">
                        <div className="flex flex-col">
                          ...
                          > 
                            <option key={"t-1"} value={" "}>{" "}</option>
                            <option key={"t-2"} value={1}>Training</option>
                            <option key={"t-3"} value={3}>Game</option>
                            <option key={"t-4"} value={2}>Assessment</option>
                          </select>
                        </div>
                      </td>
                      <td className="text-sm pr-6 whitespace-no-wrap text-gray-800 dark:text-gray-100 tracking-normal leading-4">
                        <div className="flex flex-col">
                          <label className="mb-3 text-sm leading-none text-gray-800">Event name</label>
                          <input 
                            type="text" 
                        ...
                        <div className="flex flex-col">
                          ...
                          >
                            ...
                            {dataset?.trainers?.map((trainer, index)=> {
                              return <option value={trainer.id} key={index}>{trainer.firstName} {trainer.lastName}</option>
                            })}
                          </select>
                        </div>
                      </td>
                  ...
                  <>
                    {events?.map((event, index)=> {
                      ...
                      <tr key={index} className="h-24 border-gray-300 border-b border-t hover:border-indigo-300 hover:shadow-md cursor-pointer transition duration-150 ease-in-out">
                        ...
                        <td className="text-sm pr-6 whitespace-nowrap text-gray-800 dark:text-gray-100 tracking-normal leading-4 text-center">
                        ...
                        <td className="text-sm pr-6 whitespace-nowrap text-gray-800 dark:text-gray-100 tracking-normal leading-4">
                          <div className="flex items-center">
                            ...
                            <button className="mx-2 rounded border border-transparent focus:outline-none focus:border-gray-800 focus:shadow-outline-gray" href="javascript: void(0)">
                              <div 
                                className="p-2 bg-gray-100 hover:bg-gray-200 rounded cursor-pointer text-slate-700" 
                                onClick={() => navigate(`/playerevaluation/${events[index].id}`)}
                              >
                          ...
                          <td className="py-16 pl-8 pr-6 text-left whitespace-no-wrap text-sm text-gray-800 dark:text-gray-100 tracking-normal leading-4">
                            {events[index]?.assignments?.length > 0 && 
                              <>
                                ...
                                {events[index].assignments.map((assignment, assignment_index) => {
                                  const player = dataset?.players?.filter(player => player.id === assignment.player_id)[0];
                                  if (player) {
                                    return <div className="flex mt-3" key={assignment_index}> 
        ...
        {events?.length > 0 && 
          <div className="mx-auto container pt-8 flex justify-center sm:justify-end items-center">
            ...
            <p className="text-gray-800 dark:text-gray-100 fot-normal text-base">of 1</p>
